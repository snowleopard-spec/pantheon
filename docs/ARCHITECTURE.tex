...
\begin{tikzpicture}[
  ...
  outbox/.style={rectangle, rounded corners=2pt, draw=orange!55!black, fill=orange!15, minimum width=2.8cm, minimum height=0.95cm, align=center, text width=2.7cm, inner sep=3pt},
  ...
  \node[outbox, right=of s3] (o1) {\ttfamily data/minidex.db\\{\scriptsize SQLite, all stages}};
  \node[outbox, below=of o1] (o2) {\ttfamily data/raw/\\{\scriptsize cached Item 1 text}};
  \node[outbox, right=of s7] (o3) {\ttfamily outputs/<date>/\\{\scriptsize weights.csv/parquet + manifest.json}};
...
\section{Downstream: prices and returns}

The pipeline stops at frozen weights. Two standalone scripts sit downstream to close the loop from ``these are the members and their weights'' to ``these are the returns.'' They are outside the SQLite pipeline (no DB writes) and can be run independently against any historical \texttt{minidex\_weights.csv}.

\begin{tabular}{@{}p{3.5cm}p{11.5cm}@{}}
\toprule
\textbf{Script} & \textbf{What it does} \\
\midrule
\texttt{scripts/pull\_prices.py} & Reads a weights CSV, extracts the unique ticker list, and fetches adjusted daily closes from Polygon.io with concurrent HTTP workers. Writes \texttt{data/prices.csv}. Handles rate limits and unknown-ticker responses. Requires \texttt{POLYGON\_API\_KEY} in \texttt{.env}. \\
\addlinespace
\texttt{scripts/bucket\_returns.py} & Reads the weights CSV and prices CSV, computes trailing \textbf{1y / 6m / 3m / 1m / 1w} returns per bucket via $\sum($weight\_score $\times$ ticker\_return$)$, with per-window renormalisation across members that priced. Windows are laid out longest-first to match how performance readouts are usually scanned. Writes a stylish standalone HTML report at \texttt{outputs/<asof>/bucket\_returns.html} titled ``Pantheon'', sorted by 1-year return. The report is \textbf{interactive}: each bucket row is clickable and expands to reveal a constituent table (ticker, full company name, weight, LLM score, market cap formatted as \$T/\$B/\$M, confidence) sorted by weight descending, with a chevron indicator that flips on expand. Company names are looked up at render time from \texttt{data/minidex.db} (\texttt{companies.name}); an optional \texttt{-{}-db} flag overrides the path, and the report falls back to ticker-only if the DB is unavailable. Expand/collapse is pure inline JavaScript --- no external dependencies. \\
\bottomrule
\end{tabular}

\vspace{0.6em}
Weight scheme is configurable via \texttt{-{}-weight-col} (\texttt{weight\_score} default; also \texttt{weight\_cap\_score} or \texttt{weight\_equal}). Any of the three columns can be used to answer ``how did this theme perform?'' from different angles. This is where user iteration lives --- the heavy lifting (SEC fetch, embedding shortlist, LLM scoring) already produced the CSV; deriving custom weighting schemes or return calculations is a small pandas exercise with no pipeline re-run required.

\vspace{2em}
\hrule
\vspace{0.5em}
\noindent{\small\itshape Generated for the mini-dex project. See \texttt{docs/MINIDEX\_SPEC.md} for the full technical specification, \texttt{definitions/minidex\_definitions.yaml} for the 22 bucket definitions, \texttt{prompts/scoring\_prompt.md} for the current scoring rules, \texttt{docs/OUTPUT\_COLUMNS.md} for the CSV column reference, and \texttt{README.md} for the run sequence.}
