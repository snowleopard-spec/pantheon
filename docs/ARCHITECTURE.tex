...
\addlinespace
\texttt{scripts/bucket\_returns.py} & Reads the weights CSV and prices CSV, computes trailing \textbf{1w / 1m / 3m / 6m / 1y} returns per bucket via $\sum($weight\_score $\times$ ticker\_return$)$, with per-window renormalisation across members that priced. Writes a stylish standalone HTML report at \texttt{outputs/<asof>/bucket\_returns.html} sorted by 1-year return. \\
\bottomrule
\end{tabular}
