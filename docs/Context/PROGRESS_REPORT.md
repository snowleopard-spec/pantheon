...
- Spec revised at the gate (user request): added §6 file plan — the compute layer goes in a **new** `scripts/report_metrics.py` (+ `tests/test_report_metrics.py`, `assets/vendor/` uPlot files) precisely so file ownership stays disjoint, with `bucket_returns.py`/`pull_prices.py`/`config.json` as the only edited code files — and §7 parallel execution plan (Wave 1: three agents on disjoint files; Wave 2: single-owner `bucket_returns.py` integration; Wave 3: docs + adversarial-review agents; deploy stays with the main session).
