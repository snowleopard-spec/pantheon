...
**v2.4 shipped (2026-08-07):** merged + tagged `v2.4`; droplet pulled and refreshed clean — tinted rows verified in the served page. Nightly cycle now carries the z column. Also logged: the FPGA/PQC universe audit (background agent) found **no LWLG-style SIC exclusions** — all five US-listed FPGA makers are in-universe under SIC 3674; LSCC's 10-K PQC language was seen and correctly zeroed under Rule 4 (no quantified revenue); the one gap is upstream — INTC never cleared the embedding shortlist for any bucket and AMD only via its fabless anchor, so a *forced-shortlist* mechanism (pin (ticker, bucket) pairs into scoring) is queued as a possible future item, distinct from `TICKER_INCLUDE`. Outstanding: phone check.

## 20. v2.5 deep-score list (2026-08-08)

Spec drafted 2026-08-07: `PANTHEON_V2.5_SPEC.md` — a `deep_score` list in the definitions yaml (seeded INTC, AMD) whose members bypass the embedding shortlist entirely, are scored against all 22 buckets, and run on **Claude Opus 5** (config `deep_score_model`) instead of Haiku — same v1.6 prompt, same batch submission (per-request models), ~$0.15 per seed re-score. Key resolved decisions: Opus 5 over Fable (price, data-retention requirement, batch refusal handling); explicit supersession of other-model rows on ingest rather than relying on `latest_scores`' accidental string ordering; no anchor-style privileges — membership still earned through score + floor. Companion explainer added: `docs/Explainers/TEMPERATURE_EXPLAINED.md` (user request — what temperature is, why the spec pinned 0, why Opus 5 requests must omit it).

### Implementation (2026-08-08, branch `v2.5-deep-score`)

Built per spec §3, all offline-tested before any pipeline run:

- `definitions/minidex_definitions.yaml`: top-level `deep_score: [INTC, AMD]` with the admission-criterion comment.
- `minidex/config.py` + `config.json`: `deep_score_model` setting (default `claude-opus-5`), resolved env `MINIDEX_DEEP_SCORE_MODEL` > config.json > default like its siblings.
- `minidex/shortlist.py`: `load_deep_score_tickers()`; deep members get all-bucket pairs unconditionally with `source='deep'` (real similarity recorded for debugging). Written **before** the anchor pass, so an anchor pair keeps its `anchor` provenance (AMD × fabless). Non-candidate deep tickers log-and-skip.
- `minidex/score.py`: per-request models — deep tickers get `deep_score_model` with the Opus 5 request shape (**no `temperature`** — Opus 5 400s on any sampling param — and `max_tokens=16_000` since thinking is on by default and shares the cap); everyone else's Haiku requests are byte-identical to before (unit-asserted). Cost estimate prices each request at its own model's batch rates (Opus 5 $2.50/$12.50 per MTok with the 50% discount vs Haiku $0.50/$2.50). Poll: `stop_reason == "refusal"` recorded as a loud failure per D5; ingesting deep-model rows deletes that company's other-model rows at the same `prompt_version` (logged count) per D6. `retry()` routes deep tickers identically.
- Tests: +8 (deep loader, all-22-pairs-below-threshold, anchor-beats-deep provenance, non-candidate skip; deep request shape + Haiku byte-identity, yaml-driven routing in `submit`, refusal-as-failure, supersession delete scoped to the one company). Suite: **236 passed** + the 6 pre-existing anchor-data failures.
- Docs: `OUTPUT_COLUMNS.md` gains the deep-score note under `model_version` (provenance + D8 cross-model caveat) and the updated row-selection rule.

### Seed run + rebuild (2026-08-08)

- `filter` skipped as a verified no-op (both tickers already candidates with FY2025 filings). `shortlist` (Mac, scoring extras): **44 deep pairs written** — INTC 22/22 `deep`, AMD 21 `deep` + fabless kept as `anchor`. INTC's best similarity across all 22 buckets is **0.428** (memory_storage) — direct confirmation of the audit finding that the 0.60 gate structurally could not see it.
- **Environment note:** this Mac recompute found only 610 above-threshold embed pairs vs the droplet's historical 2,844 (same model id, same threshold) — a Mac-vs-droplet embedding discrepancy worth remembering if shortlist is ever re-run for real on the Mac. No rows were lost (the stage upserts; embed total stayed 3,408) and scoring reads the table as-is, so it didn't affect this build.
- `score submit --tickers INTC,AMD -y`: 1 batch, 4 requests, all on `claude-opus-5` (submit confirmation showed the mixed-model note and per-model pricing). Batch `msgbatch_018BXqX1KGBAn2Ekkk2Z2Wcj`, ended in ~3 min, 4/4 succeeded, no refusals. Poll ingested **88 rows, 0 failures** and logged the D6 supersession: *"superseded 2 row(s) from other models for AMD at prompt v1.6."*
- **Scores (two-run averages):** AMD — fabless_chip_design **1.0 high** (up from Haiku's 0.85-band anchor row), microcontrollers 0.1, all else 0. INTC — fabless_chip_design **0.45 medium** (above the 0.25 floor → Intel enters an index on merit), foundry **0.125 low** (below floor — exactly the D7 expectation for Intel Foundry's small external revenue), microcontrollers/networking 0.05, all else 0. Opus's INTC runs used ~2× the thinking tokens of AMD's — the harder judgment got more compute, which is what the deep list is for.
- QC at baseline (same 6 anchor fails; borderline 141 → 145 from the new sub-floor rows). `build --asof 2026-08-08`: **342 rows / 22 buckets** (+INTC in fabless_chip_design at 2.17% weight_score; AMD holds 4.83% with score 1.0). Promoted to `definitions/`, archive `outputs/2026-08-08/` tracked. Prices: INTC full history fetched (10 HTTP calls total, cache did the rest). Render verified: search payload now 812 companies (was 810; +LWLG earlier, +INTC now), both names with full 22-score sets, INTC member row + 21 below-floor, correct `model_version` provenance in the weights CSV.
- **Actual Anthropic spend: $0.237** (33.5k in / 12.2k out at Opus 5 batch rates) — acceptance §5.4 (<$1) met with room; the $0.86 submit-time estimate was the 16k-max_tokens ceiling, real thinking+JSON output came in at ~3k tokens/request.
- Acceptance §5: all four criteria verified (22×`claude-opus-5` rows each incl. AMD supersession; search panel complete; suite green with Haiku requests byte-identical; spend under $1).

### Shipped (2026-08-08)

Merged fast-forward to `main`, tagged **`v2.5`**, pushed. Droplet: `git pull` + refresh republished all three pages 08:49 UTC.

**Deploy gotcha found and fixed:** the droplet's `data/minidex.db` is not git-tracked, and the search panel is built from it at render time — after the pull, the served page had INTC's constituent row (weights CSV travels via git) but no INTC search entry, and it turned out **LWLG's search scores had been silently missing from the droplet page since v2.2's DB snapshot** for the same reason. Fixed by `scp data/minidex.db` to the droplet + re-refresh: search now carries 812 companies on both machines (verified INTC + LWLG in the served payload, 08:51 UTC). **Lesson: any run that changes `latest_scores` (new tickers, re-scores) needs a DB copy to the droplet alongside the git pull — the git-based weights flow alone only updates the index tables.** Outstanding: phone check.

### v2.5.1 — the item1_path portability bug (2026-08-08, same day)

Investigating the "Mac-vs-droplet embedding discrepancy" flagged above uncovered the real cause — not embeddings at all. **`filings.item1_path` was stored machine-absolute** (1,221 rows `/root/pantheon/...` from the droplet fetch, 78 `/Users/wessch/...` from the Mac), and `_read_item1` in both `shortlist.py` and `score.py` silently returned `""` on FileNotFoundError. Each machine could only read the filings it had fetched itself, even though the raw corpus is fully mirrored on the Mac. Diagnosis chain: model weights ruled out (single HF snapshot cached since Jul 24), lockfile ruled out (sentence-transformers 5.6.1 / transformers 5.14.1 / torch 2.13.0 identical across all history), then the path check landed it.

**Consequences, all verified:**

- The shortlist "discrepancy" fully explained: the droplet's 2,844 embed pairs came from its 1,221 readable texts; the Mac's ~610 from its 78; empty-text embeddings never cleared the 0.60 threshold (no garbage pairs entered the table). The two runs never disagreed on a shared input.
- **INTC's v2.5 Opus scores had been produced with an empty Item 1** (prompt reconstruction read 0 chars; the batch token counts corroborated — 6,299 input vs AMD's 10,460). The segment table carried the judgment, which is why the scores looked plausible.
- The droplet's full-universe scoring had the mirror problem for the 77 pilot companies — but **zero bad rows survived**: `INSERT OR IGNORE` discarded every empty-Item-1 droplet row as a PK collision with the pilot's real-text rows, and the droplet's empty embeds added no new pairs to score. Sized with an ATTACH query against `scored_v16.bak`: 0 suspect rows, 0 remediation needed.

**Fixes (v2.5.1):** `edgar.py` now stores `item1_path` repo-relative; one-time DB migration rewrote all 1,299 rows to `data/raw/...` (verified 1,299/1,299 resolve locally; snapshot `pre_pathfix.bak`). `_read_item1` now warns loudly on a missing file in both modules, and **`score submit`/`retry` hard-refuse** if any candidate's Item 1 reads empty — the silent `""` is what let this run invisibly for two weeks. Suite: 237 passed + the 6 pre-existing.

**Re-runs:** full-corpus shortlist on the Mac — the first time any machine embedded all 1,299 real texts — found **3,454 above-threshold pairs = exactly the union already in the table** (droplet's 2,844 + Mac's 564 + 46 anchor/deep overlaps; zero new pairs, zero missing-file warnings). Mac and droplet embeddings agree perfectly on shared inputs. INTC re-scored with the real 10-K ($0.13, old Opus rows deleted first — same-PK re-ingest would have been silently ignored): **fabless_chip_design 0.45 unchanged** (index outcome identical), foundry 0.125 → 0.175 (still below floor), networking 0.05 → 0; rationales now cite Item 1 content (CCG $32.2B, DCAI $16.9B, IDM caveat). Rebuilt + promoted same-day archive; v2.5 total spend $0.37.

## 21. Company descriptions + Architecture header (2026-08-09 → 2026-08-10)

- **Architecture header (2026-08-09):** `docs/ARCHITECTURE.html` top-of-page restyled to match the report/coherence pages — centered 3-rem `Architecture` title + centered muted nav line (`← Report · Coherence`), tab title `Pantheon — Architecture`. The back-link stays `href="./"` in source (the renderer's local-copy rewrite matches that exact string); stale `PANTHEON_SPEC.md` reference updated to `PANTHEON_V1_SPEC.md`.
- **Company descriptions (2026-08-10):** two-line plain-language descriptions now render under every price chart (constituent rows + member search panels) and in chartless non-member search panels. Source of truth is **`definitions/company_descriptions.yaml`** — 812 entries, git-tracked, deliberately **hand-editable** (header documents the contract: nothing regenerates it wholesale; the renderer warns on stderr about index members missing an entry). Reaches the droplet by git pull alone — no DB copy involved.
- **Generation:** one-off fan-out of 28 parallel session agents (~30 companies each) reading the local Item 1 extracts (`head -c 12000`), per-agent structured output, merged + validated centrally (coverage 812/812, lengths 167–237 chars). No API spend — the cost was ~3.7M session tokens (the corpus is ~9 MB; reading it is the irreducible cost wherever it runs), which tripped the session limit mid-run once; the workflow's resume-from-cache replayed the 18 finished agents free and re-ran only the 10 failures.
- **Two YAML gotchas caught by a round-trip check before first write:** ticker `ON` parses as YAML 1.1 boolean `True` (fix: quote keys in {Y,N,YES,NO,ON,OFF,TRUE,FALSE}), and `textwrap` breaks on hyphens which block-scalar folding rejoins with a stray space ("battery- powered") — fix `break_on_hyphens=False`.
- **Data-quality finding:** 4 extracts were too thin for the agents, which flagged them honestly instead of inventing ("Extract is a fallback blob…"): INTC (TOC-only blob — meaning even the v2.5.1 INTC re-score's Item 1 was junk and its judgment rests on segment data, consistent with the unchanged scores), VLN and NBIS (foreign-filer 20-F cover pages), SVMB (VIE boilerplate). All four hand-written from DB segment data + bucket-definition context. A proper INTC Item 1 re-extraction is a known future item.
- Verified via headless-Chrome click harness (chart-row description + AAPL non-member panel description) and shipped: droplet serving the 2.23 MB report with descriptions, 15:06 UTC.
- **Polish pass (2026-08-10, user request, fan-out verified):** constituent tables drop the per-name Sharpe column (index-level Sharpe stays, footer marked "indices only"); `.chart-desc` bumped to 0.95rem neon yellow `#ccff00` (print-safe grey override); z header renamed `3M Z` → `3M Z of Sub Idx Res`; search panel now omits zero-score rows ("no floor" display keeps sub-floor nonzeros; all-zero companies — AAPL, fittingly again — get a fallback note instead of a table of 0.00s). Two parallel verification agents both independently caught a ship-blocking bug before deploy: a `\'` in the new fallback string collapsed to a bare apostrophe in the emitted JS — one character that killed the page's entire inline script (sorting, charts, search, everything). Fixed at source, all 5 script blocks node-checked, full headless matrix re-passed, deployed.

## 22. Membership overrides — DOCN out of Hyperscalers, into Neoclouds (2026-08-16)

**Trigger (user):** DigitalOcean sat in `hyperscalers` alongside five multi-trillion-dollar names and looked far more like the `neoclouds` cohort by size.

**Diagnosis before touching anything.** The pipeline had already answered the classification question: DOCN scores **0.85 high** on `hyperscalers` and **0.20 medium** on `neoclouds` (both runs agreeing exactly, `evidence_type: description_only` — DOCN reports one segment and never disaggregates GPU revenue). So it is not a mislabelled neocloud; it is a genuine public-cloud IaaS/PaaS vendor whose GPU-rental line is roughly a fifth of the business. Market cap is not a classification input anywhere in the taxonomy — it enters only as a weighting term — so "it's too small" is not a scoring argument.

The real defect is in the **definition**: leg (a) of `hyperscalers` counts *revenue from selling public cloud*, with no scale test, while the bucket's name and preamble imply one. Any competent IaaS vendor scores ~1.0. Under the report's default `weight_score` (score-normalised) that put a ~$13B company at **19.3% of the bucket — third, between Alphabet and Microsoft** — because MSFT/AMZN score only ~0.55 (their cloud legs are half of revenue) while DOCN scores 0.85 on purity. Under `weight_cap_score` it was invisible at 0.13%, so the distortion was entirely an artefact of the scheme actually being rendered.

**Why not the obvious levers.** Lowering `score_floor` 0.25 → 0.20 admits DOCN to neoclouds (it is the *only* name that enters that bucket; 11 names enter across all buckets, 337 → 348) but is a global knob for a local problem. Making DOCN a neocloud **anchor** fails outright: `qc.ANCHOR_MIN_SCORE = 0.3`, so a 0.20 anchor trips the project's own QC gate on every run — and anchors are meant to be the canonical exemplars the bucket is defined around.

**Built: a membership-override mechanism** (`overrides:` block in `definitions/minidex_definitions.yaml`, mirroring the existing `deep_score` block's "no anchor-style privileges" framing). Deliberately weaker than an anchor — **membership only, never magnitude**:

- `action: include` admits a pair below the floor; `action: exclude` drops one that clears it. Defaults to `include`.
- No QC calibration expectation, and **no weight floor** — an included member enters at its *actual* score and is weighted proportionally, so 0.20 buys a ~5% position rather than a seat at the top.
- An exclusion beats both the score floor and anchor status, but `build` warns loudly when it contradicts an anchor declaration.
- Provenance in both directions: `manifest.json` → `overrides_applied` (ticker, bucket, action, score, stated reason); includes additionally marked `is_override` in the frozen CSV (an exclusion leaves no row to flag).
- **Anti-rot guards** — `build` reconciles every declared override against the *unfiltered* score set and reports the redundant (floor would reach the same verdict unaided), the unmatched (no such scored pair), the unknown-bucket, and the unknown-action cases. So when DOCN's GPU line eventually clears 0.25, the build says the include is removable.

**Two entries added:** `DOCN → neoclouds` (include, 0.20) and `DOCN → hyperscalers` (exclude, 0.85, reason recording that this is a patch pending the definitional scale gate).

**Result — the finding that justified the whole exercise.** DOCN returned **+325.4% over the trailing year**. At 19.3% weight it was contributing ~63 of the Hyperscalers bucket's ~68.5pp 1-year return: the "Hyperscalers" line was substantially a DigitalOcean tracker. With the exclusion applied the bucket reads **+7.2%** — which is what the actual hyperscaler cohort did (GOOG +68.6%, AMZN +13.7%, MSFT −5.2%, META −24.6%, ORCL −38.6%). Post-change hyperscaler weights: META 28.1%, GOOG 26.7%, MSFT 15.4%, AMZN 15.4%, ORCL 14.3%. Neoclouds gains DOCN at 5.4%.

**Blast radius verified by diff:** one row dropped, one row added, weights re-normalised in exactly the two touched buckets, **zero scores changed**, the other 20 buckets byte-identical. No re-score, no API spend; reaches the droplet by git pull alone (weights CSV is git-tracked, no DB copy).

**Schema change:** `is_override` appended to the weights CSV/parquet (16 columns; appended so no existing column shifts, and `bucket_returns.py` reads by name). `docs/Explainers/OUTPUT_COLUMNS.md` updated.

**Tests:** 14 new in `tests/test_indices.py` (admission below floor, no weight floor vs. an anchor in the same bucket, `(ticker, bucket)` scoping in both directions, exclusion beating anchor status, manifest records, all four staleness/validation warnings, action defaulting, and a no-`overrides:`-key backwards-compatibility case). Suite: **251 passed + the 6 pre-existing anchor failures** (STM, ACMR, APLD, ETN, GEV, VST — confirmed identical on clean `main` by stash).
